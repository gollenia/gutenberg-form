...
const Inspector = ({ attributes, setAttributes }: InspectorProps) => {
	const { width, required, pattern, name, help, error } = attributes;
		...
		<InspectorControls>
			<PanelBody title={__('Data', 'gutenberg-form')} initialOpen={true}>
				...
				<SelectControl
					...
					options={[
						...
						{ value: 'honorific-suffix', label: 'Honorific suffix' },
			...
			<PanelBody title={__('Appearance', 'gutenberg-form')} initialOpen={true}>
				<RangeControl
					...
					max={6}
					min={1}
					onChange={(value) => setAttributes({ width: value ?? 6 })}
				/>
			</PanelBody>
		</InspectorControls>
	);
